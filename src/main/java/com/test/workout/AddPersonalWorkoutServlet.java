package com.test.workout;

import java.io.IOException;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import com.webapp.dao.AddPersonalWorkoutDao;

/*
 * Browser sends Http Request to Web Server
 * 
 * Code in Web Server => Input:HttpRequest, Output: HttpResponse
 * JEE with Servlets
 * 
 * Web Server responds with Http Response
 */

//Java Platform, Enterprise Edition (Java EE) JEE6

//Servlet is a Java programming language class 
//used to extend the capabilities of servers 
//that host applications accessed by means of 
//a request-response programming model.

//1. extends javax.servlet.http.HttpServlet
//2. @WebServlet(urlPatterns = "/login.do")
//3. doGet(HttpServletRequest request, HttpServletResponse response)
//4. How is the response created?

@WebServlet(urlPatterns = "/addpersonal.do")
public class AddPersonalWorkoutServlet extends HttpServlet {	
	
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;

	@Override
	protected void doGet(HttpServletRequest request, HttpServletResponse response) 
			throws IOException, ServletException {
		AddPersonalWorkoutDao addpersonal = new AddPersonalWorkoutDao();
		String username = request.getParameter("username");
		String bodypart = request.getParameter("bodypart"); 
		String fullbody = request.getParameter("fullbody");
		String difficulty = request.getParameter("difficulty");
		String workoutname = request.getParameter("workoutname");
		String videolink = request.getParameter("link");
		int reps = Integer.parseInt(request.getParameter("reps"));
		int sets = Integer.parseInt(request.getParameter("sets"));
		
		addpersonal.addWorkout(username, bodypart, difficulty, workoutname, videolink, reps, sets, fullbody);
		response.sendRedirect("/list-workouts.do");
	}
}